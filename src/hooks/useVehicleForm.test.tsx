...
const TestComponent: React.FC = () => {
  ...
  return (
    <div>
      ...
      <button onClick={() => updateFile(new File([""], "test.txt"))}>
        Update File
      </button>
      <button onClick={() => setPreset("tesla", "Model 3", "Performance")}>
        Set Preset
      </button>
...
describe("useVehicleForm", () => {
  ...
  it("provides correct make options", () => {
    ...
    expect(screen.getByTestId("makeOptions")).toHaveTextContent(
  ...
  it("provides model options when make is set", () => {
    ...
    expect(screen.getByTestId("modelOptions")).toHaveTextContent(
  ...
  it("provides badge options when model is set", () => {
    ...
    expect(screen.getByTestId("badgeOptions")).toHaveTextContent(
      JSON.stringify(expectedBadges),
    );
  });
  ...
  it("validates form correctly", () => {
    ...
    expect(screen.getByTestId("validate")).toHaveTextContent(
      "Make, Model, Badge, and Logbook file",
    );

    fireEvent.click(screen.getByText("Update Make"));
    expect(screen.getByTestId("validate")).toHaveTextContent(
      "Model, Badge, and Logbook file",
    );

    fireEvent.click(screen.getByText("Update Model"));
    expect(screen.getByTestId("validate")).toHaveTextContent(
      "Badge and Logbook file",
    );
